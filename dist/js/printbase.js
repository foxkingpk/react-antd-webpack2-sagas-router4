...
!function(document, undefined){
	...
	var printPreviewInit = kdPrintBase.printPreviewInit = function(id, machine, callback){
		//获取用户个人模板
		getMyTempById(id, function(resultJson){
			if(resultJson.data && resultJson.data.length > 0){
				//打印或预览都使用这个变量存储的模板
				printTempData = resultJson.data[0];
				//获取回单模板
				getBackTempById(printTempData.tempid, function(resultJson){
					if(resultJson.data && resultJson.data.length > 0){
						printBackTempData = resultJson.data[0];
					}
				});
				//获取模板内容设置
				getMyParamById(id, function(resultJson){
					if(resultJson.data){
						printParamData = resultJson.data;
					}else{
						printParamData = {};
					}
					if(machine){
					}else if(printTempData.machine){//个人模板有打印机信息
						machine = sqlFilter(printTempData.machine, "decode");
						printTempData.machine = machine;
					}else{
						machine = "null";
					}
					var width = printTempData.width;
					var height = printTempData.height;
					var leftmargin = 0, topmargin = 0;
					//获取打印机配置
					getMachineById(machine, function(resultJson){
						if(resultJson.data && resultJson.data.length > 0){
							printMachineData = resultJson.data[0];
							var margin = resultJson.data[0].confinfo;
							leftmargin = margin.substring(0, margin.indexOf(","));
							topmargin = margin.substring(margin.indexOf(",") + 1);
						}
						kdPrint.setTemp(null, leftmargin, topmargin, width, height);
						printPreview();
						if(callback){
							callback(printTempData, resultJson.data[0]);
						}
					});
				});
			}
	...
	var printCloudMission = kdPrintBase.printCloudMission = function(printDataList, machine, callback){
		var missionData = []
		//添加云打印任务
		var addOneCloudMission = function(i){
			var printData = printDataList[i];
			var childNum = printData.childNum;
			var returnNum = printData.returnNum;
			var note = printTempData.note;
			//获取打印机偏移
			var margin = printMachineData.confinfo;
			leftmargin = margin.substring(0, margin.indexOf(","));
			topmargin = margin.substring(margin.indexOf(",") + 1);
			...
			if(childNum){   //打印子单
				...
				if(printTempData.company != "jd"){
					missionData.push({
						title: printData.recName + printData.recmobile,
						recName: printData.recName,
						content: printNumReplace(note, printTempData.company, printData.kuaidiNum, null, printData.returnNum, 1, childNumList.length + 1),
						width: printTempData.width,
						height: printTempData.height,
				...
				for(var i in childNumList){
					missionData.push({
						title: printData.recName + printData.recmobile,
						recName: printData.recName,
						content: printNumReplace(note, printTempData.company, printData.kuaidiNum, childNumList[i], printData.returnNum, (parseInt(i) + 2), childNumList.length + 1),
						width: printTempData.width,
						height: printTempData.height,
						marginLeft: leftmargin,
						marginTop: topmargin
				...
				missionData.push({
					title: printData.recName + printData.recmobile,
					recName: printData.recName,
					content: printNumReplace(note, printTempData.company, printData.kuaidiNum, null, printData.returnNum),
					width: printTempData.width,
					height: printTempData.height,
					marginLeft: leftmargin,
					marginTop: topmargin
			...
			if(returnNum){  //打印回单
				...
				missionData.push({
					title: printData.recName + printData.recmobile,
					recName: printData.recName,
					content: printNumReplace(note, printTempData.company, returnNum),
					width: printTempData.width,
					height: printTempData.height,
					marginLeft: leftmargin,
					marginTop: topmargin
			...
			i ++;
			if(i < printDataList.length){
				addOneCloudMission(i);
			}else if(callback){
				addCloudMission(machine, missionData, callback)
			}
		}
		addOneCloudMission(0);
	}
	...
	var getFontSizeNum = function(width, height, count, lim){
		...
		fontSizeMap = [{fontNum: 8, fontWidth: 13.5, fontHeight: 16}, {fontNum: 9, fontWidth: 14.5, fontHeight: 17}, {fontNum: 10, fontWidth: 15.5, fontHeight: 19},
			{fontNum: 11, fontWidth: 17.5, fontHeight: 19}, {fontNum: 12, fontWidth: 18.5, fontHeight: 22}]; //, {fontNum: 7, fontWidth: 11.5, fontHeight: 14}, {fontNum: 13, fontWidth: 19.5, fontHeight: 24}, {fontNum: 14, fontWidth: 21.5, fontHeight:26}
	...
	//整合上两个函数，自动调整合适字体大小
	// var resizeToFit = function(content, targetStr, count){
	// 	if(!count) return content;
	// 	var reg = new RegExp('LODOP.ADD_PRINT_TEXT\\([0-9]+,[0-9]+,([0-9]+),([0-9]+),"' + targetStr + '"\\);');
	// 	var matchResult = content.match(reg);
	// 	if(!matchResult) return content;
	// 	var width = parseInt(matchResult[1]);
	// 	var height = parseInt(matchResult[2]);
	// 	var fontSizeNum = getFontSizeNum(width, height, count);
	// 	return replacePossibleFontSize(content, targetStr, fontSizeNum);
	// };
	...
	var addTemp = kdPrintBase.addTemp = function(id, account, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=addtemp&tempid=" + id + "&payaccount=" + account,
			dataType: "json",
	...
	var saveTemp = kdPrintBase.saveTemp = function(id, payaccount, gotcourier, pageWidth, pageHeight, machine, serviceType, callback){
		var leftmargin = 0, topmargin = 0;
		var modifyTemp = function(callback){
			var data = "method=modifytemp&id=" + id;
			if(pageWidth) data += "&width=" + pageWidth;
			if(pageHeight) data += "&height=" + pageHeight;
			if(payaccount) data += "&payaccount=" + encodeURIComponent(payaccount);
			else data += "&payaccount=";
			if(gotcourier) data += "&gotcourier=" + encodeURIComponent(gotcourier);
			else data += "&gotcourier=";
			if(machine) data += "&machine=" + encodeURIComponent(sqlFilter(machine, "encode"));
			else data += "&machine=";
			if(serviceType) data += "&servicetype=" + serviceType;
			else data += "&servicetype=";
			$.ajax({
				type: "post",
				url: "/snt/exp.do",
				data: data,
	...
	var getTemp = kdPrintBase.getTemp = function(com, kwd, current, size, callback){
		$.ajax({
			type: "post",
			url: "/open/snt/template.do",
			data: "method=notelist&company=" + com + "&offset=" + (current - 1) * size + "&limit=" + size + "&keyword=" + encodeURIComponent(kwd),
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getMyTemp = kdPrintBase.getMyTemp = function(kwd, current, size, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=templst&offset=" + (current - 1) * size + "&limit=" + size + "&kwd=" + encodeURIComponent(kwd),
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getMyTempById = kdPrintBase.getMyTempById = function(id, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=findtemp&id=" + id,
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getMyTempByTempid = kdPrintBase.getMyTempByTempid = function(id, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=findtemp&tempid=" + id,
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getBackTempById = kdPrintBase.getBackTempById = function(id, callback){
		$.ajax({
			type: "post",
			url: "/open/snt/template.do",
			data: "method=findbyid&needBackCheck=true&id=" + id,
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getMyParamById = kdPrintBase.getMyParamById = function(id, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=pntdetailconf&id=" + id,
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var setMyParamById = kdPrintBase.setMyParamById = function(id, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=pntdetailconf&id=" + id + "&param=" + encodeURIComponent(Obj2str(printParamData)),
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getMachineById = kdPrintBase.getMachineById = function(machine, callback){
		if(machine){
			$.ajax({
				type: "post",
				url: "/snt/exp.do",
				data: "method=pntmachineconf&act=get&machine=" + encodeURIComponent(sqlFilter(machine, "encode")),
				dataType: "json",
				success: function(resultJson){
					if(resultJson.status == 200){
						if(callback){
	...
	var setMachineById = kdPrintBase.setMachineById = function(machine, marginLeft, marginTop, callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=pntmachineconf&act=save&machine=" + encodeURIComponent(sqlFilter(machine, "encode")) + "&leftmargin=" + marginLeft + "&topmargin=" + marginTop,
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
	...
	var getEleSetting = kdPrintBase.getEleSetting = function(callback){
		$.ajax({
			type: "post",
			url: "/snt/exp.do",
			data: "method=param&param={\"type\":\"COMAUTH\"}",
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
  ...
  var getElectronicInfo = kdPrintBase.getElectronicInfo = function(data, serviceType, payaccount, success, fail){
    $.ajax({
      type:"post",
      url:"/snt/exp.do",
      data:"method=elecorderinfo&id=" + data.id + "&kuaidicom=" + data.kuaidiCom + "&payaccount=" + payaccount + "&servicetype=" + serviceType,
      dataType:"json",
      success:function(resultJson){
        if(resultJson.status == 200){
          data.payaccount = resultJson.payaccount ? resultJson.payaccount : "";                   //月结账号
          data.kuaidiNum = resultJson.kuaidinum ? resultJson.kuaidinum : "";                      //快递单号
          data.childNum = resultJson.childNum ? resultJson.childNum : "";                         //子单
          data.returnNum = resultJson.returnNum ? resultJson.returnNum : "";                      //回单
          data.bulkpen = resultJson.bulkpen ? resultJson.bulkpen : "";                            //大头笔
          data.orgCode = resultJson.orgCode ? resultJson.orgCode : "";                            //始发地区域编码
          data.orgName = resultJson.orgName ? resultJson.orgName : "";                            //始发地/始发网点
          data.destCode = resultJson.destCode ? resultJson.destCode : "";                         //目的地区域编码
          data.destName = resultJson.destName ? resultJson.destName : "";                         //目的地/到达网点
          data.orgSortingCode = resultJson.orgSortingCode ? resultJson.orgSortingCode : "";       //始发分拣编码
          data.orgSortingName = resultJson.orgSortingName ? resultJson.orgSortingName : "";       //始发分拣名称
          data.destSortingCode = resultJson.destSortingCode ? resultJson.destSortingCode : "";    //目的分栋编码
          data.destSortingName = resultJson.destSortingName ? resultJson.destSortingName : "";    //目的分栋中心名称
          data.orgExtra = resultJson.orgExtra ? resultJson.orgExtra : "";                         //始发其他信息
          data.destExtra = resultJson.destExtra ? resultJson.destExtra : "";                      //目的其他信息
          data.pkgCode = resultJson.pkgCode ? resultJson.pkgCode : "";                            //集包编码
          data.pkgName = resultJson.pkgName ? resultJson.pkgName : "";                            //集包地名称
          data.road = resultJson.road ? resultJson.road : "";                                     //路区（京东）
          data.qrCode	 = resultJson.qrCode	 ? resultJson.qrCode	 : "";                          //二维码
          data.orderNum = resultJson.orderNum ? resultJson.orderNum : "";                         //快递公司订单号
          data.expressCode = resultJson.expressCode ? resultJson.expressCode : "";                //业务类型编码（供顺丰使用）
          data.expressName = resultJson.expressName ? resultJson.expressName : "";                //业务类型名称（供顺丰使用）
          data.checkMan = resultJson.checkMan ? resultJson.checkMan : "";                         //验视人（供邮政使用）

          //停用
          /*
           //京东电子面单
           data.sourcetSortCenterId = resultJson.sourcetSortCenterId ? resultJson.sourcetSortCenterId : "";
           data.sourcetSortCenterName = resultJson.sourcetSortCenterName ? resultJson.sourcetSortCenterName : "";
           data.originalCrossCode = resultJson.originalCrossCode ? resultJson.originalCrossCode : "";
           data.originalTabletrolleyCode = resultJson.originalTabletrolleyCode ? resultJson.originalTabletrolleyCode : "";
           data.targetSortCenterId = resultJson.targetSortCenterId ? resultJson.targetSortCenterId : "";
           data.targetSortCenterName = resultJson.targetSortCenterName ? resultJson.targetSortCenterName : "";
           data.destinationCrossCode = resultJson.destinationCrossCode ? resultJson.destinationCrossCode : "";
           data.destinationTabletrolleyCode = resultJson.destinationTabletrolleyCode ? resultJson.destinationTabletrolleyCode : "";
           data.siteId = resultJson.siteId ? resultJson.siteId : "";
           data.siteName = resultJson.siteName ? resultJson.siteName : "";
           data.road = resultJson.road ? resultJson.road : "";
           data.aging = resultJson.aging ? resultJson.aging : "";
           data.agingName = resultJson.agingName ? resultJson.agingName : "";
           //韵达、圆通
           data.orgCode = resultJson.orgCode ? resultJson.orgCode : "";
           data.orgName = resultJson.orgName ? resultJson.orgName : "";
           data.destCode = resultJson.destCode ? resultJson.destCode : "";
           data.destName = resultJson.destName ? resultJson.destName : "";
           data.orgSortingCode = resultJson.orgSortingCode ? resultJson.orgSortingCode : "";
           data.orgSortingName = resultJson.orgSortingName ? resultJson.orgSortingName : "";
           data.destSortingCode = resultJson.destSortingCode ? resultJson.destSortingCode : "";
           data.destSortingName = resultJson.destSortingName ? resultJson.destSortingName : "";
           data.pkgCode = resultJson.pkgCode ? resultJson.pkgCode : "";
           data.pkgName = resultJson.pkgName ? resultJson.pkgName : "";
           data.qrCode = resultJson.qrCode ? resultJson.qrCode : "";
           data.orderNum = resultJson.orderNum ? resultJson.orderNum : "";
           */
          if(success){
            success(data);
          }
  ...
  //云打印
  var getCloudMachine = kdPrintBase.getCloudMachine = function(callback){
    var TOKEN = getcookie("TOKEN");
    $.ajax({
      type: "post",
      url: "/cloud/machine.do",
      data: "method=getshared&TOKEN=" + encodeURIComponent(TOKEN),
      dataType: "json",
      success: function(resultJson){
        if(resultJson.status == 200){
          if(callback){
	...
	var addCloudMission = function(machine, data, callback){
		var job = Obj2str(data);
		//尖括号会被拦截
		job = job.replace(/</g, "&lt;");
		job = job.replace(/>/g, "&gt;");
		...
		$.ajax({
			type: "post",
			url: "/cloud/printtask.do",
			data: "method=addjob&machineid=" + machine + "&job=" + encodeURIComponent(job),
			dataType: "json",
			success: function(resultJson){
				if(resultJson.status == 200){
					if(callback){
